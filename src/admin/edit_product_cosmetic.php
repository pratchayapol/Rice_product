...
$stmt->execute(['id' => $id]);
$products_cosmetic = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$products_cosmetic) {
    exit('ไม่พบข้อมูลผลิตภัณฑ์ที่ต้องการแก้ไข');
}

if (!empty($products_cosmetic['rice_id'])) {
...
<body class="bg t1">
    ...
    <div class="pt-24 flex items-center justify-center min-h-screen px-4">
        <div class="w-full max-w-3xl bg-white p-10 rounded-2xl shadow-xl">
            ...
            <form method="POST" action="" enctype="multipart/form-data" class="space-y-6">
                ...
                <input type="hidden" name="cosmetic_product_id" value="<?= htmlspecialchars($products_cosmetic['cosmetic_product_id']) ?>">
                ...
                <div>
                    ...
                    <input type="text" id="genbank_url" name="genbank_url" value="<?= htmlspecialchars($products_cosmetic['genbank_url'] ?? '') !== '' ? htmlspecialchars($products_cosmetic['genbank_url']) : 'รอเจ้าหน้าที่เพิ่มข้อมูล' ?>"
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        // รวม SQL UPDATE
        $sql = "UPDATE cosmetic_product SET " . implode(", ", $fieldsToUpdate) . " WHERE cosmetic_product_id = :cosmetic_product_id";
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            window.location.href = 'edit_product_cosmetic?id=<?php echo $cosmetic_product_id; ?>'; // เปลี่ยนไปหน้า accounts
                        }
                    });
                });
            </script>
