...
';
   ...
   <script>
       ...
       if (!hasValidData(chartData)) {
           ...
           for (const cat in chartData) {
               for (const field in chartData[cat]) {
                   if (fieldsToShow1.includes(field) && chartData[cat][field].length > 0) {
       ...
       const fieldNamesTH2 = {}; // เพิ่มชื่อไทยถ้าต้องการแสดงหัวกราฟสวย ๆ
       const fieldUnits2 = {}; // เช่นเดียวกันกับ fieldUnits1

       if (!hasValidData(chartDataNutrition)) {
           noDataMsg2.style.display = 'block';
       } else {
           noDataMsg2.style.display = 'none';

           let fieldsWithData = new Set();
           for (const cat in chartDataNutrition) {
               for (const field in chartDataNutrition[cat]) {
                   if (fieldsToShow2.includes(field) && chartDataNutrition[cat][field].length > 0) {
                       fieldsWithData.add(field);
                   }
               }
           }
           ...
           const categories = Object.keys(chartDataNutrition).filter(cat => {
               return fieldsToShow2.some(field => {
                   const values = chartDataNutrition[cat][field];
                   return values && values.length > 0;
               });
           });
           ...
           fieldsWithData.forEach(field => {
               ...
               const canvas = document.createElement('canvas');
               canvas.id = `chart_nutrition_${field}`;
